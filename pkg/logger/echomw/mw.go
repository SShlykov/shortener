...
import (
	...
	loggerPkg "github.com/sshlykov/shortener/pkg/logger"
...
type Config struct {
	DefaultLevel     loggerPkg.Level
	ClientErrorLevel loggerPkg.Level
	ServerErrorLevel loggerPkg.Level
...
// nolint: contextcheck
func New(logger loggerPkg.Logger) echo.MiddlewareFunc {
	return NewWithConfig(logger, Config{
		DefaultLevel:     loggerPkg.LevelInfo,
		ClientErrorLevel: loggerPkg.LevelWarn,
		ServerErrorLevel: loggerPkg.LevelError,
...
// Requests with errors are logged using slog.Error().
// Requests without errors are logged using slog.Info().
func NewWithFilters(logger loggerPkg.Logger, filters ...Filter) echo.MiddlewareFunc {
	return NewWithConfig(logger, Config{
		DefaultLevel:     loggerPkg.LevelInfo,
		ClientErrorLevel: loggerPkg.LevelWarn,
		ServerErrorLevel: loggerPkg.LevelError,
...
func NewWithConfig(logger loggerPkg.Logger, config Config) echo.MiddlewareFunc {
		...
		return func(c echo.Context) (err error) {
			...
			if err != nil {
				if _, ok := err.(*echo.HTTPError); !ok {
					err = echo.
						NewHTTPError(http.StatusInternalServerError).WithInternal(err)
				}
			}
			...
			if ok && len(xForwardedFor) > 0 {
				ips := Map(strings.Split(xForwardedFor, ","), func(ip string, _ int) string {
